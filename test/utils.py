...
def perform_val_bin_onnx(embedding_size,
                    batch_size,
                    backbone,
                    carray,
                    issame,
    ...
    with torch.no_grad():
        while idx + batch_size <= len(carray):
            # print(idx + batch_size , len(carray))
            batch = torch.tensor(carray[idx:idx + batch_size])
            if tta:
                ccropped = batch
                fliped = torch.flip(ccropped, dims=[3]).numpy()
                ccropped = batch.numpy()
                emb_batch = backbone.forward2(ccropped)+ backbone.forward2(fliped)
                embeddings[idx:idx + batch_size] = l2_norm(torch.from_numpy(emb_batch))
            else:
                ccropped = batch
                embeddings[idx:idx + batch_size] = l2_norm(backbone.forward2(ccropped.numpy()))
            idx += batch_size
        if idx < len(carray):
            batch = torch.tensor(carray[idx:])
            if tta:
                ccropped = batch
                fliped = torch.flip(ccropped, dims=[3])
                emb_batch = backbone.forward2(ccropped.numpy()) + backbone.forward2(fliped.numpy())
                embeddings[idx:] = l2_norm(torch.from_numpy(emb_batch))
            else:
                ccropped = batch
                embeddings[idx:] = l2_norm(backbone.forward2(ccropped.numpy()))
