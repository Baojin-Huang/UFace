...
def parse_args():
    ...
    parser.add_argument('--data_root', default='/sd/datasets/faces_web_mask_index',  help='validation data root')
...
def get_val_data_from_bin(data_path):
    lfw, lfw_issame = get_val_pair_from_bin(data_path, 'lfw-masked.bin')
    cfp_fp, cfp_fp_issame = get_val_pair_from_bin(data_path, 'cfp_fp-masked.bin')
    agedb_30, agedb_30_issame = get_val_pair_from_bin(data_path, 'agedb_30-masked.bin')
...
def main():
    ...
    accuracy_rmfrd, best_threshold_rmfrd = perform_val_bin(
        args.embedding_size,
        args.batch_size,
        backbone,
        rmfrd,
