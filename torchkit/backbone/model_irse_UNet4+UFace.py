...
class BasicBlockIR_hc(Module):
    ...
    def forward(self, x):
        ...
        xc0 = x[:, :int(ch / 2), :, :]
        xc1 = x[:, int(ch/2):,:, :]
        x0 = xc1[:, :, :int(4 * height / 7), :]
        x1 = xc1[:, :, int(4 * height / 7):, :]
        shortcut = self.shortcut_layer(x)
        res_c0 = self.res_layer3(xc0)
        res0 = self.res_layer1(x0)
        res1 = self.res_layer2(x1)
        res_c1 = cat([res0,res1],dim=2)
...
class Backbone(Module):
    def __init__(self, input_size, num_layers, mode='ir'):
        ...
        if input_size[0] == 112:
            self.output_layer1 = Sequential(BatchNorm2d(output_channel),
                                           Dropout(0.4), Flatten(),
                                           Linear(output_channel * 7 * 4, 256+128),
                                           BatchNorm1d(256+128, affine=False))
            self.output_layer2 = Sequential(BatchNorm2d(output_channel),
                                           Dropout(0.4), Flatten(),
                                           Linear(output_channel * 7 * 3, 256-128),
                                           BatchNorm1d(256-128, affine=False))
        else:
